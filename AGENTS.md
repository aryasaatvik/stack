...
- `sync` with no branch scopes to the current stack when the current branch is stack-relevant. Off-stack (trunk checkout or detached HEAD) it auto-scopes when exactly one stack exists, no-ops when none exist, and otherwise errors listing the stack roots and hinting `sync <branch>` or `sync --all`. Scope is resolved from a cheap membership graph (stored links plus open request bases, a request base winning over a stored parent) before any merge-base work, so scoped syncs never reconcile out-of-scope stacks.
